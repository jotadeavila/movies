...
import { API_KEY, TMBD_BASE_IRL } from '../utils/constants'
...
export const getGenres = createAsyncThunk('netflix/genres', async () => {
    const { data: {genres} } = await axios.get(`${TMBD_BASE_IRL}/genre/movie/list?api_key=${API_KEY}`)
    return genres
})
...
const createArrayFromRawData = (array, moviesArray, genres) => {
    ...
    array.forEach(movie => {
        ...
        if(movie.backdrop_path){
             moviesArray.push({
                id: movie.id,
                movie: movie?.original_name ? movie.original_name : movie.original_title,
...
export const fetchMovies = createAsyncThunk('netflix/trending', async ({type}, thunkApi) => {
    ...
    return getRawData(`${TMBD_BASE_IRL}/trending/${type}/week?api_key=${API_KEY}`, genres, true)
})

export const fetchDataByGenre = createAsyncThunk('netflix/moviesByGenres', async ({genre, type}, thunkApi) => {
    const {netflix:{genres}} = thunkApi.getState()
    return getRawData(`${TMBD_BASE_IRL}/discover/${type}?api_key=${API_KEY}&with_genres=${genre}`, genres)
})
...
const NetflixxSlice = createSlice({
    name: 'Netflix',
    initialState,
